...
public class Window
{
    ...
    public Window(MutableList<? extends String> colSpec, RichIterable<Pair<Enum, String>> sorts, Frame frame, String name)
    {
        this(colSpec, sorts.toList().collect(c -> new SortInfo(c.getTwo(), SortDirection.valueOf(c.getOne()._name()))).toList(), frame, name);
    }

    public Window(MutableList<? extends String> colSpec, RichIterable<Pair<Enum, String>> sorts, Frame frame)
    {
        this(colSpec, sorts, frame, null);
    }

    public Window(MutableList<? extends String> colSpec, MutableList<SortInfo> sorts, Frame frame, String name)
    {
    ...
    {
        CoreInstance nameInstance = window.getValueForMetaPropertyToOne("name");
        String name = nameInstance == null ? null : nameInstance.getName();
    ...
    public CoreInstance convert(ProcessorSupport ps, Frame.PrimitiveBuilder builder)
    {
        ...
        if (getName() != null)
        {
            result.setKeyValues(Lists.mutable.with("name"), Lists.mutable.with(builder.build(getName())));
        }
